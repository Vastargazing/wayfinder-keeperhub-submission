#!/usr/bin/env python3
"""Legacy scheduler compatibility. Its unavailable combined upstream step exits 3; use judge_checks.py for the snapshot suite."""
import argparse
import importlib
import json
import os
...
def main():
    ...
    ap.add_argument('--keeperhub',type=Path,default=ROOT/'keeperhub/upstream')
    ...
    names = {'reconcile-help','integration','hosted','moonwell','console',
             'moonwell-gate','calldata','mutations','patches'}
    ...
    commands=[
        ...
        ('patches',['integration/scripts/check_upstream_patches.py','--sdk',str(args.sdk),'--keeperhub',str(args.keeperhub),'--out',str(out/'patches')]),
    ]
    results=[]
    exit_code=0
